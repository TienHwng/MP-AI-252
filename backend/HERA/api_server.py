...
async def handle_assistant_message(request: web.Request) -> web.Response:
	...
	try:
		...
		if stream:
			...
			await sse_response.write(f"data: {json.dumps(jsonable(error_response))}\n\n".encode())
			await sse_response.write_eof()
			return sse_response
		return web.json_response(
			jsonable(error_response),
			status=502,
		)
	if stream:
		return await _stream_assistant_response(request, response, source)
	return web.json_response(
		jsonable(
			{
				"ok": True,
...
async def _stream_assistant_response(
	...
	try:
		...
		if isinstance(getattr(response, "metadata", None), dict):
			thinking_text = str(response.metadata.get("thinking") or "")
		for chunk in _chunk_text(thinking_text, chunk_size=3):
			chunk_data = {"choices": [{"delta": {"thinking": chunk}, "index": 0}]}
			await sse_response.write(f"data: {json.dumps(chunk_data)}\n\n".encode())
			await asyncio.sleep(0.003)

		content_text = response.text or ""
		for chunk in _chunk_text(content_text, chunk_size=1):
			chunk_data = {"choices": [{"delta": {"content": chunk}, "index": 0}]}
			await sse_response.write(f"data: {json.dumps(chunk_data)}\n\n".encode())
		...
		await sse_response.write(
			f"data: {json.dumps({'response': jsonable(final_payload)})}\n\n".encode()
		)
		await sse_response.write(b"data: [DONE]\n\n")
		await sse_response.write_eof()
		
	except (ClientConnectionResetError, BrokenPipeError, ConnectionResetError) as e:
		# Client disconnected during streaming - normal, log and return
		print(f"[STREAM] Client disconnected: {type(e).__name__}")
		return sse_response
	except asyncio.CancelledError:
		# Request cancelled/aborted
		print(f"[STREAM] Request cancelled by client")
		return sse_response
