"""Deterministic anomaly analysis service for HERA telemetry."""
...
class AnomalyAnalyzerService:
	"""Builds deterministic anomaly reports from live and recent telemetry."""

	def __init__(
		self,
		mqtt: MQTTService,
		telemetry_store: TelemetryStore | None = None,
		read_tool_runner=None,
	) -> None:
		self.mqtt = mqtt
	...
	def description(self) -> str:
		return "Analyzes telemetry freshness, anomaly score, and recent trends."
	...
	async def process(
			...
			"data_sources": [
				{
					...
					"args": {},
					"confidence": 1.0,
					"source": "anomaly_analyzer_service",
				},
				{
		...
		log_telemetry(
			f"AnomalyAnalyzer: type={classification['type']} severity={classification['severity']}",
			data={
