...
export const sendAssistantMessageStreaming = async (
	text,
	onChunk = () => {},
	onThinking = () => {},
	onError = () => {},
	...
	try {
		const response = await fetch(`${HERA_API_BASE_URL}/api/assistant/message?stream=true`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
		...
		if (contentType.includes('application/json')) {
			...
			if (!data.ok) {
				const errorMessage = data.error || data.message || 'Unknown assistant error';
				onError(errorMessage);
				throw new Error(errorMessage);
			}
			onChunk(data.text || '', data.text || '');
			await recordAssistantActivityLogs(data, text);
			return data;
		}
			...
			while (true) {
				...
				if (data === '[DONE]') {
					const completed = finalResponse || {
						ok: true,
						text: fullContent,
						metadata: fullThinking ? { thinking: fullThinking } : {},
					};
					await recordAssistantActivityLogs(completed, text);
					return completed;
				}
				...
				try {
					...
					console.warn('Failed to parse assistant stream chunk:', parseError);
				}
			}
		}
